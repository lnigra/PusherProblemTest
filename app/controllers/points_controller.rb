class PointsController < ApplicationController
  ...
  private
    ...
    def add_point_to_track
      puts "add_point_to_track..."
      device = LocationDevice.find( @point.id_source )
      device ||= Beacon.find( @point.id_source )
      puts "id_source:#{@point.id_source}"
      puts "device:#{device}"
      ...
      if mission.start && !mission.actual_end
        if host.class.to_s == "Platform"
          ...
          puts "pushing to channel:Platform_#{host.id} ..."
          ...
          track.add_point( @point )
          puts "pushing to channel:ChaseVehicle_#{@point.ident}"
