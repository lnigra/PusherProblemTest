...
#pusher = new Pusher('9a26b7ac1d284e250adb',
#  encrypted: true
#)
...
locChannelHandler = (data) =>
  point = [ data.message.lon, data.message.lat ]
  time = data.message.time.split( "T" )[1].split( "." )[0]
  $('#control #locDriver').html( "Point received: #{time} #{point}" )
  if not point2
...
platformChannelHandler = (data) =>
  console.log "msg rcvd:"
  console.log data
  point = [ data.message.lon, data.message.lat ]
  ident = data.message.ident
  time = data.message.time.split( "T" )[1].split( "." )[0]
  $('#control #beaconDriver').html( "Point received: #{ident} #{time} #{point}" )
  if not point1
...
window.addEventListener 'DOMContentLoaded', ->
  ...
  locChannel.bind 'new_point', locChannelHandler #(data) ->
  platformChannels = []
  for id, index in platformIds
    pIndex = index
    platformChannels.push(
      pusher.subscribe( 'Platform_' + id ).bind 'new_point', platformChannelHandler #(data) ->      
    )
    console.log platformChannels[index]
  console.log "DOMContentLoaded handled"
  return
